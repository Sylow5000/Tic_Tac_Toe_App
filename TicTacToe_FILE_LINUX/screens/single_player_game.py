...
class Single_Player_Game_Screen_Class(MDN_Screen):
    # +----------------------------------------+
    # |                                        |
    # |               Init Class               |
    # |                                        |
    ...
    def init_ui(self, *args):
        ...
        if self.mdn_app.mdn_screenmanager.current_screen.name != "single_player_game_screen":
            Clock.schedule_once(self.init_ui, .05)
            return
    ...
    def create_body_ui(self):
        ...
        player_profile_container = self.create_player_profile(self.player_info["profile_nickname"], self.player_info["profile_pic"] if self.player_info["profile_pic"] != None else self.theme_dest+self.theme["profile"]["default_profile"], self.theme_dest+self.theme["profile"]["default_char_{}".format(self.player_play_as)], .38)
        ...
        computer_profile_container = self.create_player_profile("CPU:{}".format(self.computer_levels[self.selected_computer_difficulty]), self.theme_dest+self.theme["profile"]["default_profile"], self.theme_dest+self.theme["profile"]["default_char_{}".format(self.computer_play_as)], .62)    
        # Combine Widgets
    ...
    def create_final_results(self, *args):
        ...
        final_result_message = MDN_Label(
            mdn_app = self.mdn_app,
            mdn_text = "You Win!" if self.winning_player == "P" else "You Lost!",
    ...
    def start_move_timer(self, *args):
        ...
        if self.current_turn == "C" or self.game_over: return
        ...
        if timer.stop_angle <= 0:
            ...
            self.player_total_time+= time() - self.player_turn_time_start
    ...
    def start_gameplay(self, *args):
        ...
        # If the computer starts first, then take this path
        if self.starts_first == "computer": 
            ...
            # Set the 'current_turn' to the 'player'
            self.current_turn = "P"
        else: self.player_turn_time_start = time()
    ...
    def make_player_move(self, args):
        ...
        self.player_avg_move_time.append(time() - self.player_turn_time_start)
        ...
        self.board_positions[int(player_cell_number / self.board_size)][player_cell_number % self.board_size] = "P"      
        ...
        args[0].mdn_update({"mdn_icon": self.theme_dest+self.theme["play_as"][self.player_play_as]})
        ...
        if self.check_win(self.board_positions, 'P', self.board_size, self.win_length): self.game_is_over("P")

    # make_computer_move method
    ...
    def init_game_play(self):
        ...
        """
        # Set the computer ai game_class_reference variable to this class
        self.computer_ai.moves = self.total_game_moves

        # Set the computer ai game_class_reference variable to this class
        self.computer_ai.game_class_reference = self

        # Set the computer ai computer_ai_difficulty variable to self.selected_computer_difficulty
        self.computer_ai.computer_ai_difficulty = self.selected_computer_difficulty
        ...
        # Initialize the move times
        # This makes it faster so we don't have to create a time every time it's the computer's turn
        self.init_move_times()
        Clock.schedule_once(partial(self.start_gameplay, 3), .2)

    # init_move_times method
    def init_move_times(self):
        """
            The purpose of this function is to create times for each move for the computer
        """
        # Create the variable that will hold the range for the move times
        # Depending the computer's difficulty, the move times will vary
        if self.selected_computer_difficulty == 0: move_time_range = [1, 5]
        elif self.selected_computer_difficulty == 1: move_time_range = [1, 3]
        elif self.selected_computer_difficulty == 2: move_time_range = [1, 2]

        # This variable will count how many times the 'move_time'
        move_time_over_two_count = 0

        # Clear 'self.computer_move_delays'
        self.computer_move_delays.clear()

        # Loop for the amount of many cells there are on the board
        for i in range(0, self.board_size*self.board_size):

            # Create a variable that will hold the time for each move
            time_for_move = random.randint(move_time_range[0], move_time_range[1] if move_time_over_two_count < 3 else 2)

            # If 'move_time_over_two_count' is less than 3 and 'time_for_move' is greater than 2, then add 1 to 'move_time_over_two_count'
            if time_for_move > 2 and move_time_over_two_count < 3: move_time_over_two_count+=1

            # Add the time for the move to 'self.computer_move_delays'
            self.computer_move_delays.append(time_for_move/100)
    ...
    def keep_score(self):
        ...
        if self.selected_move_timer != "-10":
            ...
            new_score = move_point - (seconds_for_player_to_move if self.current_turn == "P" else int(self.computer_move_delays[0]))
