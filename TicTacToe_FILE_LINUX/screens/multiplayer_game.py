...
class Multiplayer_Game_Screen_Class(MDN_Screen):
    ...
    def create_final_results(self, *args):
        ...
        final_result_message = MDN_Label(
            ...
            mdn_font_color = self.theme["final_results"]["win_message"]["font_color"]["P"],
        ...
        current_score = self.score_player[self.winning_player]
        ...
        # self.score_player[self.winning_player] = current_score

        fin_res_score_label_rel_container = RelativeLayout(size_hint = (.4, 1))
        fin_res_score_label = MDN_Label(mdn_app = self.mdn_app, mdn_text = "Score: {}pt".format(self.score_player[self.winning_player]), mdn_font_size = self.theme["final_results"]["results_info"]["font_size"], mdn_font_style = self.theme["final_results"]["results_info"]["font_style"], mdn_font_color = self.theme["final_results"]["results_info"]["font_color"])
        ...
        fin_res_total_time_label = MDN_Label(mdn_app = self.mdn_app, mdn_text = "Total Time: {}s".format(round(self.player_total_time[self.winning_player], 2)), mdn_font_size = self.theme["final_results"]["results_info"]["font_size"], mdn_font_style = self.theme["final_results"]["results_info"]["font_style"], mdn_font_color = self.theme["final_results"]["results_info"]["font_color"])
        ...
        fin_res_avg_move_time_label = MDN_Label(mdn_app = self.mdn_app, mdn_text = "Avg. Move Time: {}s".format(round(self.player_total_time[self.winning_player] / len(self.player_avg_move_time[self.winning_player]), 2)), mdn_font_size = self.theme["final_results"]["results_info"]["font_size"], mdn_font_style = self.theme["final_results"]["results_info"]["font_style"], mdn_font_color = self.theme["final_results"]["results_info"]["font_color"])
    ...
    def player_pressed_box(self, *args):
        ...
        self.current_turn = "C" if self.current_turn == "P" else "P"
        ...
        if not self.game_over:
            ...
            if self.selected_move_timer == "-10": self.current_second=0
    ...
    def start_move_timer(self, *args):
        ...
        if timer.stop_angle <= 0:
            ...
            self.total_game_moves[self.current_turn]+=1

            # Calculate the total amount of time
            self.player_total_time[self.current_turn]+= time() - self.player_turn_time_start
    ...
    def start_gameplay(self, *args):
        ...
        # Set the 'current_turn' to the 'player'
        self.current_turn = "C" if self.starts_first == "computer" else "P"
    ...
    def make_player_move(self, args):
        ...
        # Calculate the total amount of time
        self.player_total_time[self.current_turn]= self.player_total_time[self.current_turn] + (time() - self.player_turn_time_start)

        # Calculate the average move time
        self.player_avg_move_time[self.current_turn].append(time() - self.player_turn_time_start)
        ...
        self.game_moves.append({"player": self.current_turn, "cell_number": player_cell_number, "timer": self.mdn_screen_ids["timer_clock_label"].mdn_text["mobile"], "score": self.score_player[self.current_turn]})
    ...
    def keep_score(self):
        ...
        # Get the seconds from the timer clock label from the 'mdn_screen_ids'
        timer_clock_label_seconds = self.mdn_screen_ids["timer_clock_label"].mdn_text["mobile"]
    ...
    def reset_screen_variables(self):
        ...
        # This variable will record the total amount of time for the player to move
        self.player_total_time = {"P": 0, "C": 0}

        # This will record how many moves it took for the player to move and the times for each. Each time represents a move, because every time displays how long it took for the player to move. So the more times there are in this variable, that the player moved a lot
        self.player_avg_move_time = {"P": [], "C": []}
