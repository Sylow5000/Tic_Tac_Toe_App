...
class CoverBehavior(object):
    ...
    def _aspect_ratio_approximate(self, size):
        ...
        return Decimal('%.2f' % (float(size[0]) / size[1])) if float(size[0]) != 0 else 0
    ...
    def calculate_cover(self, *args):
        ...
        # scale x
            ...
            crop_size = list(self._scale_size(self.reference_size, (size[0], None)))
        ...
        # self.cover_size = offset <-
        # ORIGINAL LINE ABOVE... ^
        # NEW UPDATED LINE BELOW...
        self.cover_pos = [offset[0]+self.pos[0],offset[1]+self.pos[1]] # <-
